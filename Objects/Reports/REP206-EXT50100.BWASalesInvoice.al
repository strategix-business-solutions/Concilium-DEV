...
report 50100 "BWA Sales - Invoice"
{
    ...
    DefaultLayout = RDLC;
    //RDLCLayout = './Objects/Reports/Layouts/BWA_Sales_Invoice.rdlc';
    RDLCLayout = './Objects/Reports/Layouts/STD_Sales Invoice Report.rdl';
    WordLayout = './Objects/Reports/Layouts/BWA_Sales_Invoice.docx';
    Caption = 'BWA Sales - Invoice';
    ...
    dataset
    {
        dataitem("Sales Invoice Header"; "Sales Invoice Header")
        {
            ...
            column(Division; DimSetEntry."Dimension Value Name")
            {
            }
            ...
            column(CustVat; Cust."VAT Registration No.")
            {
            }
            column(CustPhone; Cust."Phone No.")
            {
            }
            column(CoulrDesc; "Sales Invoice Header"."BWA Colour Description")
            {
            }
            ...
            dataitem(CopyLoop; "Integer")
            {
                ...
                dataitem(PageLoop; "Integer")
                {
                    ...
                    column(YourReference_SalesInvHdr; "Sales Invoice Header"."External Document No.")
                    {
                    }
                    ...
                    dataitem("Sales Invoice Line"; "Sales Invoice Line")
                    {
                        ...
                        column(BWA_Die_No; GvDieNumber)
                        {//FCV
                        }
                        column(BWA_Die_Description; GvDieDescription)
                        {//FCV
                        }
                        column(BWA_Variant_Code; GvVariantCode)
                        {//FCV 
                        }
                        column(BWA_Variant_Description; GvVariantDescription)
                        {//FCV 
                        }
                        column(BWA_Lenght; GvLenght / 1000)
                        {//FCV 
                        }
                        column(BWA_Sqr_Piece; GvSquarePiece)
                        {//FCV 
                        }
                        column(BWA_Price_Sqr; GvPriceSqr)
                        {//FCV 
                        }
                        column(BWA_Price_M; GvPriceLenght)
                        {//FCV 
                        }
                        ...
                        begin
                            ...
                            GvSquarePiece := 0;
                            GvVariantCode := '';
                            GvVariantDescription := '';

                            if "Sales Invoice Line".Type = "Sales Invoice Line".Type::Item then begin
                                GrecItem.get("No.");
                                if (GrecDieReg.get(GrecItem."BWA Die No.")) and
                                   (GrecItemVariant.get(GrecItem."No.", "Sales Invoice Line"."Variant Code")) then begin
                                    GvDieNumber := GrecDieReg."Die No.";
                                    GvDieDescription := GrecDieReg.Description;
                                    GvLenght := GrecItem."BWA Lenght";
                                    GvSquarePiece := GrecItem."BWA Square Meter";
                                    GvVariantCode := GrecItemVariant.Code;
                                    GvVariantDescription := GrecItemVariant.Description;
                                    GcuBWAmngmnt.fCalcPricing(GrecItem."No.",
    ...
    begin
        ...
        SaveAsPdf(StrSubstNo('%1 - %2', "Sales Invoice Header"."No.", "Sales Invoice Header"."Bill-to Customer No."));
    end;
    ...
    var
        ...
        GrecDieReg: Record "BWA Die Register";
        GrecItem: Record Item;
        GrecItemVariant: Record "Item Variant";
        GvDieNumber: Code[20];
        GvDieDescription: Text[50];
        GvVariantCode: Code[10];
        GvVariantDescription: code[50];
        GvLenght: Decimal;
        GvSquarePiece: Decimal;
        GvPriceSqr: Decimal;
        GvPriceLenght: Decimal;
        GcuBWAmngmnt: Codeunit "BWA Management";
    ...
    local procedure fDieRegister(PvItemNo: Code[20]; var PrecDieRegister: Record "BWA Die Register"): Boolean
    var
        LrecItem: Record item;
    begin
        exit((not (LrecItem.get(PvItemNo))) and (not (PrecDieRegister.Get(LrecItem."BWA Die No."))));
    end;
